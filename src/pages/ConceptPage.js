...
const ConceptPage = () => {
  ...
  const raw = getConcept(slug);
  const concept = raw ? localizeConcept(raw, lang) : null;
  ...
  const related = getRelatedConcepts(raw).map((c) => localizeConcept(c, lang));
  const { prev, next } = getAdjacent(raw);
